    ...
    <fu-form size="mini" :rules="rules" :fu-data="config">
      <div style="display: flex">
        ...
        <fu-form-item label="执行器" label-width="80px" prop="jobGroup">
          <fu-select
            v-model="config.jobGroup"
            placeholder="请选择"
            :disabled="isJobGroupDisabled"
          >
            <fu-option
...
export default {
  ...
  watch: {
    config: {
      handler(newVal) {
        if (newVal) {
          this.isJobGroupDisabled =
            "HttpJob,EmailJob".indexOf(newVal.taskType) > -1 ? true : false;
        }
      },
      deep: true,
      immediate: true,
    ...
    getOptions() {
      postJSON("/api/schedulecenter/v1/jobgroup/queryAllGroup.do").then(
        (res) => {
          ...
          if ("HttpJob,EmailJob".indexOf(this.config.taskType) > -1) {
            let findItem = result.find(
              (item) => item.text == "xxl-job-executor-local"
            );
            if (findItem) {
              this.$set(this.config, "jobGroup", findItem.value);
            }
          }
        }
      );
