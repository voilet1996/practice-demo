...
const checkRules = (json = {}) => {
  ...
  if (baseCheck.result) {
    baseCheck = checkRulesSql(json)
  }
  return baseCheck
}
function checkRulesSql(json) {
  let result = {
    result: true,
    message: "校验通过",
  }
  if (json.SQLJob.connectionPool === "") {
    result.result = false
    result.message = "请选择连接池"
  } else if (json.SQLJob.sqlText === "") {
    result.result = false
    result.message = "请输入SQL"
  }
  return result
}
